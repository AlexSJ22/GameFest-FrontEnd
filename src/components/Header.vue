...
<template>
  ...
  >
    ...
    <ul class="hidden lg:gap-9 lg:text-xl md:flex gap-5 md:text-xs text-white items-center">
      <li class="enlace"><RouterLink to="/">Inicio</RouterLink></li>
      <li class="enlace"><RouterLink to="/games">Juegos</RouterLink></li>
      <li class="enlace"><RouterLink to="/events">Eventos</RouterLink></li>
      ...
      <li v-else class="relative group">
        ...
        <!-- Dropdown menu -->
        <div class="absolute right-0 mt-2 w-48 bg-zinc-900 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-purple-500/30">
          <button 
            @click="handleLogout"
    ...
    <!-- Botón menú móvil -->
    <button @click="toggleMenu" class="md:hidden text-white p-2 rounded-lg hover:text-orange-500 transition-colors z-[100] ml-auto relative">
    ...
    >
      <div v-show="isOpen" class="md:hidden absolute top-full left-1/2 -translate-x-1/2 w-[90%] mt-4 rounded-3xl overflow-hidden shadow-2xl z-40">
        <div class="bg-gradient-to-b from-purple-900 to-blue-900 backdrop-blur-sm px-6 py-4 space-y-1 border border-red-900/50">
          ...
          <button
            v-if="!authStore.isLoggedIn"
            @click="() => { closeMenu(); modalStore.openLogin(); }"
