...
const fetchGameDetail = async () => {
  loading.value = true
  try {
    const response = await api.get('games.php', { params: { id: props.id } })
  ...
  <AnimatedModal :open="isOpen" @update:open="isOpen = $event">
    <AnimatedModalBody class="md:max-w-4xl w-full !bg-black border border-purple-500/30 shadow-2xl overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row min-h-[450px]">
          ...
          <div class="md:w-3/5 p-8 space-y-6 flex flex-col justify-center bg-black">
            ...
            <div class="flex gap-2">
              <span class="px-3 py-1 bg-purple-500/20 border border-purple-500/50 text-purple-400 rounded-full text-xs uppercase">{{ game.genero }}</span>
            </div>
      ...
      <AnimatedModalFooter v-if="game" class="!bg-black border-t border-white/10">
        <button @click="isOpen = false" class="w-full px-6 py-4 bg-zinc-800 text-white rounded-xl font-bold hover:bg-zinc-700 border border-zinc-700">
          Cerrar
        </button>
      </AnimatedModalFooter>
    </AnimatedModalBody>
