...
// IMPORTS
// ----------------------
import { ref, onMounted, computed } from 'vue' // Hooks de Vue: reactividad y ciclo de vida
import api from '@/lib/api'                     // Instancia Axios personalizada
import LiquidBackground from '@/components/ui/LiquidBackground.vue' // Fondo líquido animado
import BendingGallery from '@/components/ui/BendingGallery.vue'     // Componente para galería curvada

// ----------------------
// REACTIVITY - DATOS
// ----------------------
const events = ref([])        // Array de eventos cargados desde API
const loading = ref(true)     // Estado de carga
const total = ref(0)          // Total de eventos en la API

// Estado del formulario
const formStarted = ref(false)  // Indica si se inició el formulario
const currentStep = ref(1)      // Paso actual del formulario (1 a 4)
const eventCreated = ref(false)  // Indica si el evento fue creado exitosamente
const uploading = ref(false)     // Estado de subida de imagen
const selectedImageFile = ref(null) // Archivo de imagen seleccionado
const imagePreview = ref(null)      // Preview en base64 de la imagen
...
const uploadImage = async () => {
  ...
  try {
    ...
    // POST a la API para subir imagen
    const response = await api.post('/events/upload-image.php', formDataUpload, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })

    if (response.data.success) {
      return response.data.filename // Retorna nombre del archivo subido
    } else {
...
const eventImages = computed(() => {
  // Array para la galería: solo URL de imagen y texto vacío
...
const totalEvents = computed(() => events.value.length) // Total de eventos cargados

const mostCommonType = computed(() => {
  // Calcula el tipo de evento más repetido
...
const recentEvent = computed(() => {
  // Devuelve el evento más reciente por fecha
  if (events.value.length === 0) return null
  
  const sorted = [...events.value].sort((a, b) => new Date(b.fecha) - new Date(a.fecha))
  return sorted[0]
})
...
const getTipoStyles = (tipo) => {
  // Clases Tailwind según tipo de evento
  const styles = {
...
// ----------------------
const startForm = () => { formStarted.value = true; currentStep.value = 1 }
...
const createEvent = async () => {
  try {
    ...
    if (response.data.success) {
      ...
      await fetchEvents() // refresca eventos
    } else {
...
const createAnother = () => {
  // Reset parcial para crear otro evento
  formData.value = { titulo:'', tipo:'', fecha:'', hora:'', plazas:'', imagen:'', descripcion:'' }
...
// CICLO DE VIDA
// ----------------------
onMounted(fetchEvents) // Carga eventos al montar el componente
</script>
...
<template>
  ...
  <div>
    ...
    <!-- ==========================================================
         SECCIÓN DE CABECERA CON FONDO LÍQUIDO
         Contiene el título principal "Crear Evento"
    ========================================================== -->
    <div class="relative h-[60vh] sm:h-[70vh] md:h-[70vh] w-full overflow-hidden">
      
      <!-- Componente animado de fondo líquido -->
      <LiquidBackground class="absolute inset-0" />

      <!-- Overlay oscuro semitransparente para contraste con el título -->
    ...
    <!-- ==========================================================
         SECCIÓN PRINCIPAL: Galería de Eventos y Formulario
    ========================================================== -->
    <div class="py-6 px-4 sm:py-12 sm:px-6 lg:px-10 max-w-[1400px] mx-auto">
      ...
      <!-- ==========================================================
           INDICADOR DE CARGA
           Muestra un mensaje mientras se cargan los eventos
      ========================================================== -->
      <div v-if="loading" class="flex justify-center items-center py-20">
        <p class="text-purple-500 font-['Pixelify_Sans'] text-2xl animate-pulse">
          Cargando eventos...
        </p>
      </div>
      ...
      <!-- ==========================================================
           GALERÍA DE EVENTOS
           Se muestra si no está cargando
      ========================================================== -->
      <div v-else class="relative flex h-96 w-full flex-col items-center justify-center gap-24 overflow-clip rounded-xl border border-zinc-800 mb-8 sm:mb-12">
        <!-- Array de imágenes de eventos -->
         <!-- Curvatura de la galería -->
          <!-- Radio de bordes de cada imagen -->
        <BendingGallery
          :items="eventImages"       
          :bend="2"                  
          :border-radius="0.1"       
      ...
      <!-- ==========================================================
           ...
           Solo se muestran cuando no se está cargando
      ========================================================== -->
      <div v-if="!loading" class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 sm:mb-12">
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-xl p-6 backdrop-blur-sm group hover:scale-105 transition-all duration-300">
          <!-- Decoración circular en esquina superior derecha -->
        ...
        <div v-if="recentEvent" class="relative overflow-hidden bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl p-6 backdrop-blur-sm group hover:scale-105 transition-all duration-300">
          ...
          <!-- Overlay degradado oscuro para contraste -->
          ...
          <div class="relative z-10">
            <div class="flex items-center justify-between mb-4">
              ...
              <!-- Etiqueta tipo evento con color dinámico -->
              <span 
      ...
      </div> <!-- Fin tarjetas de estadísticas -->

      <!-- ==========================================================
           SECCIÓN FORMULARIO CREAR EVENTO
      ========================================================== -->
      <div class="mb-6">
        <h2
          class="text-3xl sm:text-4xl font-bold font-['Pixelify_Sans'] bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent">
      ...
      <!-- Contenedor principal del formulario paso a paso -->
      <div class="bg-zinc-900/50 border border-zinc-800 rounded-lg p-6 sm:p-8 md:p-12 min-h-[400px] flex items-center justify-center">
        ...
        <!-- ==========================================================
             TRANSICIÓN ANIMADA ENTRE PASOS DEL FORMULARIO
        ========================================================== -->
        <transition
          mode="out-in"
        ...
        >
          <!-- Aquí van los distintos pasos del formulario -->

          <!-- Paso inicial: botón para comenzar el formulario -->
          ...
          <!-- ============================
               Pantalla de éxito
               Se muestra cuando el evento se ha creado
          ============================ -->
          <div v-else-if="eventCreated" key="success" class="text-center space-y-8">
            ...
            <div class="relative">
              ...
              <!-- Efecto de animación "ping" detrás del icono -->
        ...
        </transition> <!-- Fin de transición de pasos -->
